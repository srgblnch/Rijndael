...
def unitTest(debug=False):
    ...
    Logger(debug=debug).debug_stream("state",state,0,"cipher->addRoundKey()\t")
    Logger(debug=debug).debug_stream("subkey",subkey,0,"cipher->addRoundKey()\t")
    srv._addRoundKey.do(state,subkey)
    Logger(debug=debug).debug_stream("state",state,0,"cipher->addRoundKey()\t")
    ...
    if not unitTest:
        debug_stream("state", unit,operation="should be\t\t\t")
    ...
    state = [[0x19,0xa0,0x9a,0xe9],
             ...
             [0xe3,0xe2,0x8d,0x48],
             [0xbe,0x2b,0x2a,0x08]]
    Logger(debug=debug).debug_stream("state",state,0,"cipher->subBytes()\t")
    srv._subBytes.do(state)
    Logger(debug=debug).debug_stream("state",state,0,"cipher->subBytes()\t")
    ...
    print("Unit Test= %s"%unitTest)
    if not unitTest:
        Logger(debug=debug).debug_stream("state", unit,operation="should be\t\t\t")
    ...
    state = [[0xd4,0xe0,0xb8,0x1e],
             ...
             [0x11,0x98,0x5d,0x52],
             [0xae,0xf1,0xe5,0x30]]
    Logger(debug=debug).debug_stream("state",state,0,"cipher->shiftRows()\t")
    state = srv._shiftRows.do(state)
    Logger(debug=debug).debug_stream("state",state,0,"cipher->shiftRows()\t")
    unit = [[0xd4,0xe0,0xb8,0x1e],
            [0xbf,0xb4,0x41,0x27],
            [0x5d,0x52,0x11,0x98],
    ...
    state = [[0xd4,0xe0,0xb8,0x1e],
             ...
             [0x5d,0x52,0x11,0x98],
             [0x30,0xae,0xf1,0xe5]]
    Logger(debug=debug).debug_stream("state",state,0,"cipher->MixColumns()\t")
    state = srv._mixColumns.do(state)
    Logger(debug=debug).debug_stream("state",state,0,"cipher->MixColumns()\t")
