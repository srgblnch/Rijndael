...
def int2key(argin):
    return _Long(8).toArray(argin, 4*6*8)


def key2int(argin):
    return _Long(8).fromArray(argin, 4*6*8)
...
class AES192:
    ...
    def cipher(self):
        ...
        self._round = 0
        self.addRoundKey()
        for self._round in range(1, 12):  # ith round ---
            self.subBytes()
            self.shiftRows()
            self.mixColumns()
            self.addRoundKey()
        ...
        output = gRijndael(aes192['key'],
                           nKeyColumns=6).cipher(aes192['input'])
        if output != aes192['output']:
            raise AssertionError(["Failed AES192: output doesn't correspond "
                                  "(%s)" % (hex(output))])
    ...
    def decipher(self):
        ...
        if len(self._Errors) > 0:
            raise AssertionError(self._Errors)
        # Finally, test the Rijndael object ---
        input = gRijndael(aes192['key'],
                          nKeyColumns=6).decipher(aes192['output'])
        if input != aes192['input']:
            raise AssertionError(["Failed AES192: output doesn't correspond "
                                  "(%s)" % (hex(input))])
    ...
    def addRoundKey(self):
        subkey = self._keyExpansionObj.getSubKey(self._round*4,
                                                 (self._round+1)*4)
        if subkey != int2subkey(aes192_round[self._round]['k_sch']):
        ...
        if matrix2int(self._state) != aes192_round[self._round]['end']:
            self._Errors.append("Failed AES192: round%d, addRoundKey()"
                                % (self._round))
            self._state = int2matrix(aes192_round[self._round]['end'])

    def subBytes(self):
        self._state = self._subBytesObj.do(self._state)
        if matrix2int(self._state) != aes192_round[self._round]['s_box']:
            self._Errors.append("Failed AES192: round%d, subBytes()"
                                % (self._round))
            self._state = int2matrix(aes192_round[self._round]['s_box'])

    def shiftRows(self):
        self._state = self._shiftRowsObj.do(self._state)
        if matrix2int(self._state) != aes192_round[self._round]['s_row']:
            self._Errors.append("Failed AES192: round%d, shiftRows()"
                                % (self._round))
            self._state = int2matrix(aes192_round[self._round]['s_row'])

    def mixColumns(self):
        self._state = self._mixColumnsObj.do(self._state)
        if matrix2int(self._state) != aes192_round[self._round]['m_col']:
            self._Errors.append("Failed AES192: round%d, mixColumns()"
                                % (self._round))
            self._state = int2matrix(aes192_round[self._round]['m_col'])

    def invShiftRows(self):
        self._state = self._shiftRowsObj.invert(self._state)
        if matrix2int(self._state) != aes192_round[self._round]['is_row']:
            self._Errors.append("Failed AES192: round%d, invShiftRows()"
                                % (self._round))
            self._state = int2matrix(aes192_round[self._round]['is_row'])

    def invSubBytes(self):
        self._state = self._subBytesObj.invert(self._state)
        if matrix2int(self._state) != aes192_round[self._round]['is_box']:
            self._Errors.append("Failed AES192: round%d, invSubBytes()"
                                % (self._round))
            self._state = int2matrix(aes192_round[self._round]['is_box'])

    def invAddRoundKey(self):
        subkey = self._keyExpansionObj.getSubKey((12-self._round)*4,
                                                 (12-self._round+1)*4)
        if subkey != int2subkey(aes192_round[self._round]['ik_sch']):
            self._Errors.append("Failed AES192: round%d, getSubKey()"
                                % (self._round))
            subkey = int2subkey(aes192_round[self._round]['ik_sch'])
        self._state = self._addRoundKeyObj.do(self._state, subkey)
        if matrix2int(self._state) != aes192_round[self._round]['ik_add']:
            self._Errors.append("Failed AES192: round%d, invAddRoundKey()"
                                % (self._round))
            self._state = int2matrix(aes192_round[self._round]['ik_add'])

    def invMixColumns(self):
        self._state = self._mixColumnsObj.invert(self._state)
        if matrix2int(self._state) != aes192_round[self._round]['iend']:
            self._Errors.append("Failed AES192: round%d, invMixColumns()"
                                % (self._round))
            self._state = int2matrix(aes192_round[self._round]['iend'])
